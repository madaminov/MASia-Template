...
var Datepicker = (function () {
  ...
  function calcTraditionalWeekNumber(date, weekStart) {
    ...
    const weekOneOfNextYear = dayOfTheWeekOf(
      new Date(date).setDate(32),
      weekStart,
      weekStart
    );
        ...
        const getWeekNumber =
          typeof method === 'function'
            ? (timeValue, startOfWeek) =>
                method(new Date(timeValue), startOfWeek)
            : determineGetWeekMethod(
  ...
  class DaysView extends View {
    ...
    refreshFocus() {
      this.changeFocusedCell(
        Math.round((this.focused - this.start) / 86400000)
      );
    }
  }
  ...
  class MonthsView extends View {
    ...
    setOptions(options) {
      ...
      if (options.checkDisabled) {
        this.checkDisabled =
          this.isMinView || options.datesDisabled === null
  ...
  class Picker {
    constructor(datepicker) {
      ...
      const [prevButton, viewSwitch, nextButton] =
    ...
    place() {
      ...
      if (orientX === 'auto') {
        if (inputLeft < scrollAreaLeft) {
          ...
          orientX =
            inputRight - calendarWidth < scrollAreaLeft ? 'left' : 'right';
        } else {
          orientX = 'left';
        }
      }
      ...
      if (orientY === 'auto') {
        if (inputTop - calendarHeight > scrollAreaTop) {
          orientY =
            inputBottom + calendarHeight > scrollAreaBottom ? 'top' : 'bottom';
        } else {
          orientY = 'bottom';
        }
      }
    ...
    setViewSwitchLabel(labelText) {
      ...
      //custom html
      if (/\s/.test(labelText)) {
        let labelText_arr = labelText.split(' ');
        this.controls.viewSwitch.innerHTML =
          '<span class="month">' +
          labelText_arr[0] +
